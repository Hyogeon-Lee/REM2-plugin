...
| `prompt-enhancer` | 거친 프롬프트를 모델별(Fable 5 / GPT-5.5 / Opus 4.8 / Sonnet 5) 복사-붙여넣기용 프롬프트로 재작성 — 원본 프롬프트를 신뢰하지 않는 데이터로 취급, 프롬프트 안의 작업은 절대 실행하지 않음, 주입 시도는 무력화 후 `Neutralized Injection Attempts`로 보고. 계층 방어: 행동 규칙 + `disallowed-tools` 하드 가드(스킬 활성 중 셸·파일 쓰기·웹·서브에이전트 도구 제거) | stable |
